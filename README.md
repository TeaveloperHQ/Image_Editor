...
| 이미지 편집 | 자르기(드래그 선택)·회전/반전·텍스트/그림/도장을 **객체로 올려 이동·크기조절 후 적용** |
...
2. 작업 모드 선택:
   ...
   - **도장** — 도장 이미지를 만들어 현재 이미지에 찍거나 PNG로 저장
3. 회전(↺/↻)·좌우/상하 반전은 버튼으로 즉시 적용
4. `저장…` 으로 내보내기 (PNG/JPG 등)

#### 도장 만들기

- **글자로** — 한글/한자를 입력하면 배경 투명 도장(원형/사각, 붉은색·푸른색)으로 생성 (막도장 수준)
- **스캔 투명화** — 스캔한 직인 이미지의 흰 배경을 투명하게 변환 (실제 직인용 핵심 기능, 색 통일 옵션)
- 만든 도장은 `PNG로 저장`하거나 `이미지에 찍기`로 현재 이미지/PDF에 사용
